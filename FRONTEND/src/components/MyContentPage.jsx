...
const [loader , setloader] = useState(true)
  ...
  const getvideos = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:4000/api/v1/users/mycontent",
        {
          headers: {
      ...
      // console.log("my content ",response)
 ...
 if (loader){
  ...
  return (
    <section className="w-full pb-[70px] sm:ml-[70px] sm:pb-0 lg:ml-0">
     <h1 className='text-center mt-2 font-bold'>MY CONTENT</h1>
      <div className="flex flex-col gap-4 p-4">
       ...
       const timeAgo = formatDistanceToNow(new Date(vid?.createdAt), { addSuffix: true });
        ...
        <div className="w-full max-w-3xl gap-x-4 md:flex">
          ...
          <Link to={`/watch/${vid?._id}`}>
            <div className="w-full pt-[56%]">
              <div className="absolute inset-0"><img
                ...
                className="absolute bottom-1 right-1 inline-block rounded bg-black px-1.5 text-sm">{vid?.duration}</span>
            </div></Link>
          </div>
          <div className="flex gap-x-2 md:w-7/12">
            ...
            <div className="w-full">
              <h6 className="mb-1 font-semibold md:max-w-[75%]">{vid?.title}</h6>
              <p className="flex text-sm text-gray-200 sm:mt-3">{vid?.views?.length} Views · {timeAgo}</p>
              ...
              <p className="mt-2 hidden text-sm md:block h-6 overflow-hidden">{vid?.description}</p>
            </div>
          </div>
        </div>
         )
