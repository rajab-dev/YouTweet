...
function WatchHistoryPage() {
  ...
  const getvideos = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:4000/api/v1/users/history",
        {
          headers: {
      ...
      // console.log("history videos",response)
      setvideos(response.data.recentlywatchedvideos )
    } catch (error) {
        console.log(error)
    } finally {
  ...
  return (
    <section className="w-full pb-[70px] sm:ml-[70px] sm:pb-0 lg:ml-0">
     <h1 className='text-center mt-2 font-bold'>WATCH HISTORY</h1>
      <div className="flex flex-col gap-4 p-4">
       ...
       const timeAgo = formatDistanceToNow(new Date(vid?.video?.createdAt), { addSuffix: true });

         return(       
        <div className="w-full max-w-3xl gap-x-4 md:flex">
          <div className="relative mb-2 w-full md:mb-0 md:w-5/12">
          <Link to={`/watch/${vid?.video?._id}`}>
            <div className="w-full pt-[56%]">
              <div className="absolute inset-0"><img
                ...
                className="absolute bottom-1 right-1 inline-block rounded bg-black px-1.5 text-sm">{vid?.video?.duration}</span>
            </div></Link>
          </div>
          <div className="flex gap-x-2 md:w-7/12">
            ...
            <div className="w-full">
              <h6 className="mb-1 font-semibold md:max-w-[75%]">{vid?.video?.title}</h6>
              <p className="flex text-sm text-gray-200 sm:mt-3">{vid?.video?.views?.length} Views · {timeAgo}</p>
              <Link to={`/channel/${vid?.owner?.username}`}> 

              <div className="flex items-center gap-x-4">
                <div className="mt-2 hidden h-10 w-10 shrink-0 md:block"><img
                    src={vid?.owner?.avatar}
                    alt="codemaster" className="h-full w-full rounded-full" /></div>
                <p className="text-sm text-gray-200">{vid?.owner?.fullname}</p>
              </div></Link>
              <p className="mt-2 hidden text-sm md:block h-6 overflow-hidden">{vid?.video?.description}</p>
            </div>
          </div>
        </div>
         )
