...
function SingleTweet({tweet}) {
 ...
 const [tweetlikes, setlikes] = useState([])
 const [tweetdislikes, setdislikes] = useState([])
 ...
 const timeAgo = formatDistanceToNow(new Date(tweet.createdAt), { addSuffix: true });
   ...
   const handlelikebtn = async (tweetid) => {
      ...
      // setdislikes(response.data.dislikedby)
  ...
  return (
    <>
       ...
       <img
                ...
                alt="React Patterns" class="h-full w-full rounded-full" />
                </div>
            <div class="w-full">
